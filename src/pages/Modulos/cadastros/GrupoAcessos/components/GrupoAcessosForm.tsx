...
const GrupoAcessosForm: React.FC<GrupoAcessosFormProps> = ({ grupo, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const isEdit = Boolean(grupo?.id);
      const endpoint = isEdit ? `/api/grupos-acesso/${grupo.id}` : "/api/grupos-acesso";
      ...
      alert(`Grupo de acesso ${isEdit ? 'atualizado' : 'cadastrado'} com sucesso!`);
