...
const CargosForm: React.FC<CargosFormProps> = ({ cargo, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const endpoint = isEdit ? `/cargos/${cargo.id}` : '/cargos';
      const method = isEdit ? 'put' : 'post';

      const response = await api[method](endpoint, formData);
      const result = response.data;
      alert(`Cargo ${isEdit ? 'atualizado' : 'cadastrado'} com sucesso!`);
      if (onSave) {
        onSave(result?.data ?? formData);
      }
      if (!isEdit) {
        setFormData({ nome: '', descricao: '', nivel_acesso: 'baixo', ativo: true });
      }
    } catch (error) {
