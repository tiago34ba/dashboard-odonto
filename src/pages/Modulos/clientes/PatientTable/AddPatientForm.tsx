...
const AddPatientForm: React.FC<AddPatientFormProps> = ({ onClose, onAddPatient }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const result = await onAddPatient(encryptedPatient);
      if (!result.success) {
        setErrors([result.errorMessage || 'Não foi possível gravar no MySQL via API. Verifique os dados e tente novamente.']);
      }
    } catch (error) {
